...
public class PointsTeamAllowedHome {
        ...
        for(String position: positions)
        {
			...
			while(rs.next())
			{
				...
				for(int i =0; i < matchups.size(); i++)
				{
					...
					query2 = "select * from game where matchupID='"+matchups.get(i) +"'and location='"+team+"' and date<20170112";
				...
				//System.out.println(fantasy/games.size()+ "fantasy"+ fantasy+"  size is:"+ games.size());
				query2="update team set "+position + "AHOME='"+ df.format(fantasy/games.size()) + "' where abbriviation='"+team+"'";
